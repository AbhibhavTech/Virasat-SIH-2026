...
export const IndiaHierarchyPage: React.FC<IndiaHierarchyPageProps> = ({
  ...
  // Valid Cities / Towns in current state (must have at least one verified place)
    ...
    return db.states.filter((state) => {
      ...
      if (selectedRegion !== 'All') {
        if (selectedRegion === 'Union Territories') {
          matchesRegion =
            state.id.includes('delhi') ||
            state.id.includes('ladakh') ||
            state.id.includes('jammu') ||
            state.id.includes('chandigarh') ||
            state.id.includes('puducherry') ||
            state.name.toLowerCase().includes('territory');
        } else {
  ...
  const searchResults = useMemo(() => {
    ...
    const matchedStates: Array<{ id: string; name: string; region: string }> = [];
    ...
    for (const state of db.states) {
      if (state.name.toLowerCase().includes(q) || state.capital.toLowerCase().includes(q)) {
        matchedStates.push({ id: state.id, name: state.name, region: state.region });
      }
  ...
  return (
    <div className="min-h-screen bg-[#FAF8F5] text-[#0B192C] pb-16">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 sm:pt-8 space-y-6 sm:space-y-8">
              ...
              {activeLevel === 'india' && (
                <div>
                  <h1 className="font-serif text-2xl sm:text-4xl font-bold text-[#0B192C] tracking-tight">
                    Explore India — Heritage, Culture & Local Gems
                  </h1>
                  <p className="text-xs sm:text-sm text-[#6B5E55] max-w-3xl leading-relaxed mt-1.5">
                    Browse every Indian State & Union Territory to uncover famous World Heritage sites alongside authentic, lesser-known historic towns, archaeological wonders, and living traditions.
                  </p>
                </div>
              )}
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              {activeLevel !== 'india' && (
                ...
                >
                  ...
                  <span>Back to {activeLevel === 'city' ? currentState.name : 'All States'}</span>
                </button>
              )}
            </div>
          </div>

          {/* Universal Search & Instant Autocomplete Dropdown */}
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-4">
              ...
              <div className="relative w-full md:w-96">
                ...
                <input
                  ...
                  placeholder="Search state, small town, or place (e.g. Lepakshi, Majuli, Bodh Gaya)..."
                        ...
                        <div className="space-y-1">
                          {searchResults.states.map((s) => (
                            ...
                            >
                              ...
                              <span className="text-[11px] text-[#7A6E65]">{s.region}</span>
                            </button>
                          ))}
                        </div>
                      </div>
                    ...
                    {searchResults.states.length === 0 &&
                      ...
                      searchResults.places.length === 0 && (
                        <div className="p-4 text-center text-xs text-stone-500">
                          No states, towns, or heritage places matched "{searchQuery}".
                        </div>
                      )}
                  </div>
                )}
        ...
        {/* =========================================================================
            LEVEL 1: ALL INDIA VIEW (STATE CARDS GRID OR MAP EXPLORER)
        ========================================================================= */}
        {activeLevel === 'india' && (
          <>
            {viewMode === 'map' ? (
                  ...
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <div className="text-xs font-bold text-stone-900">
                        Accurate Real-World India Geographic &amp; Satellite Map Available
                      </div>
                      <div className="text-[11px] text-stone-600">
                        View real satellite &amp; street cartography mapped with verified ASI &amp; UNESCO coordinates across all states.
                      </div>
                    </div>
                  </div>
                  <button
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-bold uppercase tracking-wider text-[#FF671F]">
                      States &amp; Territories ({filteredStates.length})
                    </span>
                    <span className="text-xs text-stone-500 font-medium">
                      Select any state to explore its heritage towns &amp; cultural places
                    </span>
                  </div>
                </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="p-5 flex flex-col justify-between grow space-y-4">
                            ...
                            {/* Verified Towns / Local Discovery Highlights */}
                            ...
                            >
                              <span>Explore {state.name} Towns</span>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-[#7A6E65] mt-0.5">
                    Showing verified historic cities, sacred pilgrimage centers, and hidden gems ({stateValidCities.length}).
                  </p>
                </div>
              </div>
        ...
        {activeLevel === 'city' && currentCity && (
          <div className="space-y-8">
            {/* Town Hero & Travel Context Banner */}
            <div className="bg-white rounded-3xl border border-[#EFE8DF] overflow-hidden shadow-xs">
              <div className="grid grid-cols-1 lg:grid-cols-12">
                <div className="lg:col-span-5 relative h-64 lg:h-auto bg-stone-100">
                  <img
                      ...
                      >
                        <div>
                          ...
                          <div className="relative h-48 w-full overflow-hidden bg-stone-100">
                            <img
                              src={attr.image_url || attr.thumbnail_url}
                              alt={attr.name}
