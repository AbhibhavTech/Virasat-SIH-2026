              ...
              <div className="space-y-4">
                ...
                {filteredCityPlaces.length === 0 ? (
                  ...
                  <div className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                      {filteredCityPlaces.slice(0, placesDisplayLimit).map((attr, aIdx) => {
                              ...
                              <div>
                                ...
                                <div className="p-5 space-y-3">
                                  <p className="text-xs text-[#5A4E46] leading-relaxed line-clamp-2">
                                    {(attr as any).description || attr.summary}
                                  </p>

                                  {/* Quick Metrics */}
                                  <div className="grid grid-cols-2 gap-2 text-xs pt-1 border-t border-[#EFE8DF]">
                                    <div className="p-2 rounded-xl bg-[#FAF8F5] border border-[#EFE8DF] space-y-0.5">
                                      <div className="text-[10px] font-bold text-[#FF671F] uppercase flex items-center gap-1">
                                        <Clock className="w-3 h-3" /> Time
                                      </div>
                                      <span className="text-[11px] text-stone-700 font-semibold truncate block">
                                        {(attr as any).suggested_duration || '1–2 hrs'}
                                      </span>
                                    </div>
                                    <div className="p-2 rounded-xl bg-[#FAF8F5] border border-[#EFE8DF] space-y-0.5">
                                      <div className="text-[10px] font-bold text-[#FF671F] uppercase flex items-center gap-1">
                                        <IndianRupee className="w-3 h-3" /> Entry
                                      </div>
                                      <span className="text-[11px] text-stone-700 font-semibold truncate block">
                                        {(attr.entry_fee as any)?.domestic !== undefined
                                          ? (attr.entry_fee as any).domestic === 0
                                            ? 'Free'
                                            : `₹${(attr.entry_fee as any).domestic}`
                                          : 'Public'}
                                      </span>
                                    </div>
                                  </div>
