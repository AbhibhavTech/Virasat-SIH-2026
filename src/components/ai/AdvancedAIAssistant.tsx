    ...
    <div className="space-y-6">
      ...
      <div className="p-6 sm:p-8 rounded-3xl bg-white border border-[#EFE8DF] shadow-3d-card flex flex-col md:flex-row items-center gap-6">
        <div className="shrink-0 flex flex-col items-center">
          <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-[#FF671F] to-[#046A38] flex items-center justify-center text-white shadow-warm">
            <Compass className="w-8 h-8 text-amber-200 animate-spin-slow" />
          </div>
          <span className="mt-2 text-xs font-bold text-[#FF671F] bg-orange-50 px-3 py-0.5 rounded-full">
            AI Cultural Concierge
          </span>
        </div>

        <div className="space-y-3 text-center md:text-left flex-1">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-orange-50/70 text-[#FF671F] text-xs font-bold border border-orange-200">
          ...
          <div className="flex items-center gap-2 flex-wrap justify-center md:justify-start pt-2">
            {coreActions.map((action, i) => (
              <button
                ...
                className="px-3.5 py-2 rounded-xl bg-[#FAF8F5] hover:bg-orange-50/70 border border-[#EFE8DF] hover:border-orange-300 text-xs font-bold text-stone-800 hover:text-[#FF671F] transition shadow-2xs flex items-center gap-1.5"
              >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-[11px]">
        ...
        <div className="p-2.5 rounded-xl bg-white border border-orange-200/80 flex items-center gap-2 text-[#FF671F] shadow-2xs">
      ...
      <div className="flex flex-col h-[580px] rounded-3xl bg-white border border-[#EFE8DF] shadow-warm overflow-hidden">
        ...
        <div className="p-4 bg-[#FAF8F5] border-b border-[#EFE8DF] flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            {selectedCity && selectedCity !== 'All India' && (
              <span className="text-[10px] px-2 py-0.5 rounded-full bg-orange-50 text-[#FF671F] font-semibold border border-orange-200">
                📍 {selectedCity}
              </span>
            )}
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 bg-[#FAF8F5]/30">
            ...
            >
              <div
                className={`rounded-xl flex items-center justify-center flex-shrink-0 text-xs shadow-xs ${
                  m.role === 'user'
                    ...
                    : 'w-8 h-8 bg-orange-50 text-[#FF671F] border border-orange-200'
                }`}
              >
                {m.role === 'user' ? (
              ...
              <div
                className={`max-w-[85%] sm:max-w-[78%] rounded-2xl p-4 text-xs sm:text-sm leading-relaxed ${
                  m.role === 'user'
                    ? 'bg-[#FF671F] text-white shadow-xs font-medium'
              ...
              >
                ...
                {m.role === 'assistant' && (
                  <div className="mt-3 pt-2.5 border-t border-stone-100 flex items-center justify-between text-[10px] text-stone-500">
                    ...
                    <span className="text-stone-400">Virasat Cultural Engine</span>
                  </div>
                )}
              </div>
            </div>
          ...
          {loading && (
            <div className="flex items-start gap-3">
              <div className="w-8 h-8 rounded-xl bg-orange-50 border border-orange-200 text-[#FF671F] flex items-center justify-center flex-shrink-0 shadow-xs">
        ...
        <div className="px-4 py-2 bg-stone-50 border-t border-[#EFE8DF] flex items-center gap-2 overflow-x-auto scrollbar-none">
          ...
          {sampleDestinationCards.map((dest) => (
            <div
              ...
              className="group cursor-pointer shrink-0 flex items-center gap-2 p-1.5 pr-3 bg-white rounded-xl border border-[#EFE8DF] hover:border-orange-400 shadow-2xs transition"
            >
              <img
                src={dest.imageUrl}
                alt={dest.name}
              ...
              />
              <div>
                <p className="text-[11px] font-bold text-stone-800 group-hover:text-[#FF671F] leading-tight">
                  {dest.name}
                </p>
        ...
        <div className="px-4 py-2 bg-[#FAF8F5] border-t border-[#EFE8DF] flex items-center gap-2 overflow-x-auto scrollbar-none">
          <span className="text-[10px] text-stone-500 font-semibold whitespace-nowrap flex items-center gap-1">
            <Sparkles className="w-3 h-3 text-[#FF671F]" /> Prompts:
          </span>
          {suggestionChips.map((chip, idx) => (
            <button
              key={idx}
              onClick={() => handleSend(chip)}
              className="text-[11px] px-3 py-1 rounded-full bg-white hover:bg-orange-50/70 border border-[#EFE8DF] hover:border-orange-300 text-stone-700 hover:text-[#FF671F] whitespace-nowrap transition shadow-2xs font-medium"
            >
              {chip}
            </button>
          ))}
        ...
        <div className="p-4 bg-white border-t border-[#EFE8DF]">
          ...
          >
            <input
              ...
              placeholder="Ask Virasat: Where will India take you today?"
              className="flex-1 px-4 py-3 rounded-xl bg-[#FAF8F5] border border-[#EFE8DF] text-xs sm:text-sm text-stone-900 placeholder-stone-400 focus:outline-none focus:border-[#FF671F] focus:ring-1 focus:ring-[#FF671F] transition"
            />
            <button
              type="submit"
              disabled={loading || !input.trim()}
              className="px-5 py-3 rounded-xl bg-[#FF671F] hover:bg-[#E65100] disabled:opacity-50 text-white text-xs font-bold transition shadow-xs flex items-center gap-1.5"
