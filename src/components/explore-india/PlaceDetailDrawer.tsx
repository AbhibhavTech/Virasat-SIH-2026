...
export const PlaceDetailDrawer: React.FC<PlaceDetailDrawerProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-black/60 backdrop-blur-xs animate-fadeIn">
      <div className="relative w-full max-w-2xl bg-[#FCFBF9] rounded-2xl sm:rounded-3xl border border-[#EFE8DF] shadow-2xl overflow-hidden flex flex-col max-h-[92dvh]">
        {/* Header Image or Neutral Placeholder */}
        <div className="relative h-44 sm:h-64 w-full bg-stone-200 shrink-0 overflow-hidden">
          ...
          <button
            onClick={onClose}
            className="absolute top-3.5 right-3.5 p-1.5 sm:p-2 rounded-full bg-black/50 text-white hover:bg-black/75 transition backdrop-blur-xs focus:outline-none cursor-pointer z-10"
          ...
          <div className="absolute bottom-3 left-3 right-3 sm:bottom-4 sm:left-4 sm:right-4 text-white pointer-events-none">
            <div className="flex flex-wrap items-center gap-1 mb-1 sm:mb-2">
              {place.topic && (
                <span className="px-2 py-0.5 rounded-full bg-orange-600/90 text-white text-[9px] sm:text-[10px] font-bold tracking-wider uppercase backdrop-blur-xs">
                  {place.topic}
                </span>
              )}
              {place.subtopic && (
                <span className="px-2 py-0.5 rounded-full bg-stone-800/80 text-amber-200 text-[9px] sm:text-[10px] font-medium backdrop-blur-xs">
                  {place.subtopic}
                </span>
              )}
              {isHeritage && (
                <span className="px-2 py-0.5 rounded-full bg-[#046A38] text-white text-[9px] sm:text-[10px] font-semibold flex items-center gap-1 backdrop-blur-xs">
                  <Award className="w-2.5 h-2.5 sm:w-3 sm:h-3" /> UNESCO
                </span>
              )}
            </div>

            <h2 className="font-serif text-xl sm:text-3xl font-bold leading-tight drop-shadow-sm">
              {place.name}
            </h2>

            <div className="flex items-center gap-1.5 text-[11px] sm:text-xs text-stone-200 mt-0.5">
        ...
        <div className="p-3.5 sm:p-6 overflow-y-auto space-y-4 sm:space-y-5 text-[#0B192C]">
          {/* Trust & Verification Header Status */}
          <div className="p-3 sm:p-3.5 rounded-xl sm:rounded-2xl bg-white border border-[#EFE8DF] flex flex-wrap items-center justify-between gap-2 sm:gap-3 shadow-2xs">
        ...
        {/* Footer Actions */}
        <div className="p-3 sm:p-4 bg-white border-t border-[#EFE8DF] flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2.5 shrink-0">
          <div className="flex items-center gap-2">
            <button
              onClick={() => {
                if (onOpenAIChat) {
              ...
              className="flex-1 sm:flex-initial px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl bg-[#FAF8F5] hover:bg-[#F3EDE5] border border-[#EFE8DF] text-xs font-bold text-[#E65100] transition flex items-center justify-center gap-1.5 cursor-pointer"
            >
            ...
            {hasCoordinates && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="flex-1 sm:flex-initial px-3 sm:px-3.5 py-2 sm:py-2.5 rounded-xl border border-stone-200 hover:border-amber-400 bg-white text-stone-700 hover:text-amber-800 text-xs font-semibold transition flex items-center justify-center gap-1.5 cursor-pointer shadow-2xs"
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={onClose}
              className="px-3.5 sm:px-4 py-2 sm:py-2.5 rounded-xl border border-stone-200 text-stone-700 hover:bg-stone-50 text-xs font-semibold transition cursor-pointer"
            >
              Close
            </button>
            {onSelectPlace && (
              <button
                ...
                className="flex-1 sm:flex-initial px-4 sm:px-5 py-2 sm:py-2.5 rounded-xl bg-[#FF671F] hover:bg-[#E65100] text-white text-xs font-bold transition flex items-center justify-center gap-1.5 shadow-xs cursor-pointer"
              >
