...
export const PlaceDetailDrawer: React.FC<PlaceDetailDrawerProps> = ({
  ...
  const hasValidImage = (place.image_url || place.thumbnail_url) && !imgError;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-fadeIn">
      <div className="relative w-full max-w-2xl bg-[#FCFBF9] rounded-3xl border border-[#EFE8DF] shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="relative h-56 sm:h-64 w-full bg-stone-200 shrink-0">
          ...
          {/* Photographic provenance badge if available */}
          {hasValidImage && place.creator && (
            <div className="absolute top-4 left-4 text-[10px] px-2.5 py-1 rounded-full bg-black/60 text-white font-mono backdrop-blur-xs">
              📷 {place.creator} ({place.license || 'Verified'})
            </div>
          )}
