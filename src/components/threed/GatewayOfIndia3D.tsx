...
export const GatewayOfIndia3D: React.FC<GatewayOfIndia3DProps> = ({
  ...
  return (
    <div className="relative rounded-2xl overflow-hidden bg-stone-100 border border-stone-200 shadow-xs">
      <div className="absolute top-4 left-4 z-10 flex items-center gap-2">
        <span className="px-3.5 py-1.5 rounded-full bg-white/90 backdrop-blur-md border border-stone-200 text-xs font-bold text-stone-800 shadow-xs flex items-center gap-1.5">
          ...
          <span className="text-[11px] text-stone-500 font-normal">
            {webGLFailed ? '(Archival Image)' : '(3D Simulation)'}
          </span>
        </span>
      </div>
      ...
      {webGLFailed ? (
        <div className="w-full h-80 sm:h-96 relative overflow-hidden bg-stone-900">
          <img
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-black/75 via-transparent to-transparent flex items-end p-6">
            <span className="text-white text-xs">Architectural perspective archival capture</span>
          </div>
        </div>
      ) : (
      ...
      <div className="p-3.5 bg-white border-t border-stone-200 flex items-center justify-between text-xs text-stone-600">
        <span className="flex items-center gap-1.5">
          ...
          <span>{webGLFailed ? 'Archival photography view' : 'Click & drag to rotate monument perspective freely'}</span>
        </span>
        <span className="text-[11px] font-semibold text-emerald-700">
          {webGLFailed ? 'National Archives' : 'Renderer: Three.js PBR'}
        </span>
      </div>
    </div>
  );
