    ...
    try {
      ...
      if (Array.isArray(stationsList)) {
        ...
        for (const stn of stationsList) {
          ...
          const resolvedCityId = resolveCanonicalCityId(rawCity) || stationCityToCityId[normCity] || (cities[normCity] ? normCity : undefined);
          const stnId = stn.id || `stn-${code.toLowerCase()}`;
          transit_nodes[stnId] = {
            id: stnId,
            type: 'railway',
            name: stn.name,
            ...
            city_id: (resolvedCityId && cities[resolvedCityId]) ? resolvedCityId : (stn.city_id && cities[stn.city_id] ? stn.city_id : resolvedCityId || undefined),
